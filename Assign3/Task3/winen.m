...
som = newsom(normWineInputs, [10 10], 'hextop', 'linkdist', 150, 5);
som.trainParam.epochs = 200;  

[trained_som, stats] = train(som, normWineInputs);

fprintf('The following are the plots:\n');

% Assign each data point to the best matching unit (BMU)
bmu_indices_class1 = vec2ind(trained_som(normWineInputs(:, 1:59)));
bmu_indices_class2 = vec2ind(trained_som(normWineInputs(:, 60:130)));
bmu_indices_class3 = vec2ind(trained_som(normWineInputs(:, 131:178)));
...
figure;
plotsomhits(trained_som, normWineInputs(:, 1:59));
title('Winning Nodes for Class 1');
saveas(gcf, 'normalized_class1.png');

% Plot and save Winning Nodes for Class 2
figure;
plotsomhits(trained_som, normWineInputs(:, 60:130));
title('Winning Nodes for Class 2');
saveas(gcf, 'normalized_class2.png');

% Plot and save Winning Nodes for Class 3
figure;
plotsomhits(trained_som, normWineInputs(:, 131:178));
title('Winning Nodes for Class 3');
saveas(gcf, 'normalized_class3.png');
